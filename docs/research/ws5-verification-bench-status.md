...
| CP-3/M1 **device 실측 (ALR 절대값 + native baseline)** | M1 | **PARTIAL+** — ALR 절대값 정량화(exec_ms ~18-20ms, path-xlate 4334.7 ns/op, traps=0) + **native baseline 랜딩**(microbench: compute 4.06 / syscall getpid 200.36 ns/op). 근사 raw-syscall 오버헤드 **~9%** (getpid vs getppid = **apples-to-apples 아님**, reported only). 정밀 same-binary ratio는 아래 PENDING |
...
| CP-3 native baseline + same-binary ratio | M1/M3 | **native baseline LANDED** (microbench: compute 4.06 / syscall getpid 200.36 ns/op, adb shell). 근사 raw-syscall ~9% (getpid vs getppid — apples-to-apples 아님, reported only). **정밀 same-binary ratio = NEW DEVICE-REQ**: WS-1이 동일 microbench를 ALR guest로 실행(compute+syscall) → ALR ns/op → 진짜 native-vs-ALR %. PRoot는 SELinux로 보류 |
