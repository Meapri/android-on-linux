...
| CP-3/M1 **CPU 오버헤드 (same-binary native vs ALR)** | M1 | **CLOSED** ✅ — 동일 microbench(static musl) native(adb)+ALR loader: **compute +0.00%**(ALR 4.06=native 4.06, gated <5% **PASS**), syscall(getpid) **+11.98%**(storm, reported). 일반 연산/CLI = **native급 0% 오버헤드 device-verified**. `2026-06-01-cp3-apples-to-apples-gtk3-svg-perm.md` |
...
| CP-3 same-binary ratio | M1/M3 | **CLOSED** ✅ — WS-1이 동일 microbench를 ALR guest로 실행 완료: compute ALR 4.06=native 4.06 (**0%**, gated PASS), syscall getpid ALR 224.36 vs native 200.36 (**~12%**, storm). 정밀 native-vs-ALR ratio 확보. 잔여: PRoot A/B(SELinux 보류, 부수) |
