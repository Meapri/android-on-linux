...
| 로컬 `dpkg -i hello.deb` (비root, 단일 .deb) | **INSTALLS** (`unpacked=true configured=true installed=true`; status="install ok installed"; maintainer script "Setting up hello" 실제 실행; 바이너리 `<rootfs>/usr/bin/hello` 안착) | 2026-06-03 ws-1 `7f45def` (R5KL20B6S3X, v163) | **v2 길목 device-달성** (이전 `configured=false`). 2겹 fix: fakeroot link→copy-fallback + `utimes()/lutimes()` 경로재작성 래퍼(dpkg 가 `.dpkg-new` 타임스탬프를 utimes 로 찍던 ENOENT 중단 해소), exec-child in-process re-map 워커가 B-3 augmented envp 를 x21 에 미러(재진입 자식이 interposer/fakeroot 없이 도는 것 방지). chromium `--version`/gimp-console 회귀0 |
| 실제 `apt install <pkg>` (네트워크 fetch + 다중-패키지 트랜잭션) | **WALL: 다중-패키지 maintainer-script exec-re-entry** (round-4 drain#13 device-확인: `apt-install: unpacked=false configured=false exec=GUEST EXEC FAIL`); 기능 자체 **PENDING**. 로컬 단일 `dpkg -i` 는 v2 달성(`7f45def`)이나 원격 fetch + 다중 unpack/configure 체인은 미증명; galculator 류 깊은 fork+exec 체인은 supervisor 40s watchdog 벽 잔존 | 2026-06-03 ws-1 `7f45def`; 2026-06-02-round4-milestones-drain | `dpkg -i hello.deb`(얕은 maintainer script)는 `7f45def` 로 device-증명(installed=true)됐다. 그러나 **네트워크 `apt install`**(원격 fetch + 다중 패키지 unpack/maintainer-script/트리거 폭주)은 round-4 drain#13 에서 `GUEST EXEC FAIL` 로 device-확인된 **WALL** 로 남는다 — 다중 패키지의 **maintainer-script exec-re-entry**(dpkg 가 helper 를 `fork+execve` 로 띄우고 rootfs 바이너리가 ALR 로더로 재진입) + **galculator 류 깊은 fork+exec 체인**(supervisor 40s watchdog)이 **PENDING**. `7f45def` 가 얕은 단일 .deb 길을 닫았고 후속은 그 위에서. **별개의 큰 로더 마일스톤**(clone3 PRoot 한계 메모리: device-evidence-mali-android16). 잠금해제 범위·난이도·의존: `loader-feature-gaps.md` |
...
| Qt6 (qtwayland) | Qt | **overlay STAGED, Qt-init SIGSEGV → wayland closure 보강중(round-5)** (qt6-stage overlay extracted=289 적재; round-4 drain#13 에서 `analogclock` 바이너리 staged + `QT_QPA_PLATFORM=wayland` 설정됐으나 fork 된 **게스트 child(pid 21297) 가 Qt init 중 `signal 11 SIGSEGV SEGV_MAPERR`** → `qt6gui-result: rendered=false`) | — | 2026-06-02-round4-milestones-drain | WS-4. **앱 회귀 아님**(app pid 21224 생존, crash 후 모든 후속 probe 정상 마커). 105MB overlay + wayland QPA plugin 만으로는 Qt platform init 클로저 부족 → **Qt6 wayland closure 보강**(libQt6WaylandClient + integration plugins + deps) 후 launch = **round-5 진행 중**(device 렌더 미확인). 상세: `loader-feature-gaps.md` |
...
| guest Vulkan enumerate/props 마샬링 backbone (`alr_gpu_vk_{proto,decode,marshal_probe}`) | **HOST-VERIFIED** (`native_vk_marshal_test` ALL PASS, NDK 4-ABI clean) | 2026-06-02-round4-milestones-drain | round-4 host-merged. 게스트 Vulkan enumerate/props 호출을 ring 으로 마샬링하는 backbone 이 off-device host 테스트로 검증됨(아직 실 Mali Vulkan 드라이버에 미연결). **device wiring 진행중** — JNI `run_vk_marshal_mali_probe`(`nativeHostVulkanProbe` 옆)로 실 Mali 에서 `ALR VK ENUM MARSHAL: PASS` 캡처는 1줄 후속(round-5). Vulkan render pipeline(`VK_ANDROID_external_memory_AHB` color-attach → present)은 별개 큰 작업: `loader-feature-gaps.md` |
| guest Vulkan ops over ring + ICD (VK-M2/M3, render pipeline) | PENDING | — | enumerate/props backbone 다음 단계. ring 으로 실 Vulkan 명령 마샬 + 게스트 `libvulkan_alr.so` ICD + AHB color-attach 렌더타깃. Vulkan-native 게임/Wine·DXVK 잠금해제(전략: `gpu-guest-accel-strategy.md`). 난이도·의존: `loader-feature-gaps.md` |
...
| in-app `apt`/`dpkg` 실제 설치 | **PARTIAL** — 로컬 `dpkg -i hello.deb` = installed device-달성(`7f45def`, v163); 네트워크 `apt install` + 깊은 fork+exec 체인은 PENDING | 로컬 단일 `.deb`(얕은 maintainer script)는 **device-PROVEN**(`7f45def`: unpacked=true configured=true installed=true; "Setting up hello" 실행; 위 패키지매니저 섹션 `dpkg -i hello.deb` 행 참조) — `7f45def` 가 maintainer-script exec-re-entry 길목(utimes 경로재작성 + exec-child re-map envp)을 닫았다. **여전히 PENDING**: 네트워크 `apt install`(원격 fetch + 다중 패키지 unpack/maintainer-script/트리거 폭주)과 **galculator 류 깊은 fork+exec 체인**(supervisor 40s watchdog 벽). **별개의 큰 로더 마일스톤**(dpkg clone3 PRoot 한계 메모리: device-evidence-mali-android16). SSOT: `loader-feature-gaps.md` |
