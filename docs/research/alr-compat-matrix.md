...
| Qt6 (qtwayland) | Qt | **overlay STAGED + 클로저 완전, Qt-init SIGSEGV(EGL hwintegration) → `EGL→wl_shm` 전환중(round-6)** (qt6-stage 적재; round-5 drain#14 에서 `analogclock` staged + `QT_QPA_PLATFORM=wayland` 인데도 fork 된 **게스트 child(pid 24125) 가 Qt init 중 `signal 11 SIGSEGV`** → `qt6gui-result: rendered=false`) | — | 2026-06-02-round5-vulkan-device-marshal, 2026-06-02-round4-milestones-drain | WS-4. **앱 회귀 아님**(app 생존, crash 후 모든 후속 probe 정상). round-5 가 원인을 device-규명: **overlay 미완이 아니라 EGL hwintegration** — Qt 가 wayland **EGL** client-buffer integration 을 골라 ICD 없는 `eglGetDisplay` 호출(ALR shim 은 GLES-마샬링이지 Qt 가 쓸 EGL platform 아님; `libQt6WaylandEglClientHwIntegration.so` 포함 35 reachable libs = 클로저는 완전). machine-id 주입 + `QT_WAYLAND_DISABLE_WINDOWDECORATION` 으로도 안 고쳐짐. **다음: EGL 플러그인 비활성 → wl_shm 소프트웨어 client-buffer 강제**(다른 toolkit 과 동일 cairo/SW→wl_shm 경로) = round-6 진행(device 렌더 미확인). 상세: `loader-feature-gaps.md`(G2) |
