...
| 실제 `apt install <pkg>` (네트워크 fetch+unpack) | **WALL: exec-re-entry** (round-4 drain#13 device-확인: `apt-install: unpacked=false configured=false exec=GUEST EXEC FAIL`); 기능 자체는 **PENDING**(미구현) | 2026-06-02-round4-milestones-drain | dpkg unpack/**maintainer-script exec-re-entry** 벽이 round-4 drain#13 에서 device-확인됨 — `dpkg -i` 가 helper(tar/maintainer-script)를 `fork+execve` 로 띄우는데 rootfs 바이너리의 **execve 가 ALR 로더로 재진입(exec-re-entry)** 하는 기능이 아직 미구현(PENDING). round-4 의 빠른 수정이 아니라 **별개의 큰 로더 마일스톤**(clone3 PRoot 한계 메모리: device-evidence-mali-android16). 잠금해제 범위·난이도·의존: `loader-feature-gaps.md` |
...
| in-app `apt`/`dpkg` 실제 설치 | WALL: exec-re-entry (round-4 drain#13 device-확인) | `dpkg-query`/`apt-get`/`Xwayland` **버전 보고 실행**은 device-PROVEN(drain#9). 실제 `apt install`(네트워크 fetch + dpkg unpack/maintainer-script)은 round-4 drain#13 에서 `GUEST EXEC FAIL` 로 벽 확인됨 — **maintainer-script exec-re-entry**(dpkg 가 helper 를 `fork+execve` 로 띄우고 rootfs 바이너리가 ALR 로더로 재진입) 미구현. **별개의 큰 로더 마일스톤**(빠른 수정 아님; dpkg clone3 PRoot 한계 메모리: device-evidence-mali-android16). SSOT: `loader-feature-gaps.md` |
