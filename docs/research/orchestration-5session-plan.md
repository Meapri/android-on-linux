...
**round-3 fan-out + drain#11 완료(v133 `4d7d4a9`)**: 서브에이전트 3개(ws2 GLES3 coverage·ws4 toolkit-launchable·ws5 docs) 병렬 게이트 머지(pytest 473 + NDK + shim/wire PASS) + 통합 세션이 MainActivity toolkit 경로 와이어링. drain#11: **WS-2 GLES3 코어(UBO/sampler/MRT/invalidate, 12 op) 회귀0**(LIVE/THROUGHPUT/CUBE PASS, glmark2 1067; context는 ES3.2). **toolkit: netsurf-gtk(실 GTK3 브라우저)가 ALR 로더로 PARSE/MAP/INTERP/entry까지 실행+GTK init 도달** — headless 프로브라 `cannot open display`로 exit=1(로더/의존성 실패 아님). sdl2 testver 실행됨. qt6 qtpaths6 경로 불일치. evidence: docs/evidence/2026-06-02-round3-gles3-toolkit-drain.md.
**netsurf 브라우저 render + drain#12 완료(v134 `39b2a9b`)**: netsurf-gtk(실 GTK3 웹브라우저)를 컴포지터에 디스플레이-백드로 launch → **`netsurf-result: rendered=true`(frames 2214→2217), 5 threads, 25s 풀 생존(sig14=SIGALRM, crash 아님)** = 실 웹브라우저가 ALR로 Android SurfaceView에 렌더. 회귀0(foot/gtkdemo render, glmark2 1090, SCREEN CUBE PASS). evidence: docs/evidence/2026-06-02-netsurf-browser-renders.md. 범용 GUI 증명셋에 **브라우저** 추가.
**round-4 마일스톤 fan-out + drain#13 완료(v135 `1d2fb2c`)**: 서브에이전트 5개(MainActivity launch·tools overlays·alr_gpu Vulkan·alr_wayland multiwin·docs) 병렬 게이트 머지(pytest 499 + NDK 4-ABI + native-core VK PASS) + 통합 QT_QPA_PLATFORM=wayland. drain#13: **SDL2 창 데모(testdraw2) rendered=true** = SDL2 GUI 창이 ALR로 렌더(범용셋에 SDL2 추가). **qt6 analogclock**: 바이너리 staged but Qt init에서 게스트 child SIGSEGV(rendered=false) — 앱은 생존(이후 모든 probe 정상=회귀0), Qt wayland closure 보강 필요. **gimp-filter**: gimp-console batch 실행+babl/gegl 로드(ok). **apt/dpkg install**: GUEST EXEC FAIL = dpkg가 helper fork+exec → **exec-re-entry 벽**(별개 큰 로더 기능). **Vulkan**: enumerate/props 마샬링 backbone host-merged+host-verified(device wiring 1줄 후속). 회귀0(netsurf/foot/gtkdemo render, glmark2 1047, SCREEN CUBE PASS). evidence: docs/evidence/2026-06-02-round4-milestones-drain.md.
**round-5 fan-out + drain#14 완료(v136 `28c4880`)**: 서브에이전트 4개(qt6-closure·GLES3·input·docs) 병렬 게이트 머지(pytest 519 + NDK + shim/wire) + 통합 세션이 **Vulkan device probe 배선**(runtime_report ALR_VK_DECODE_REAL + nativeAlrGpuVkMarshalProbe). drain#14: **★Vulkan enumerate 마샬링 device-검증** = guest Vulkan req→ring→host 실 Mali libvulkan→reply, `ALR VK ENUM MARSHAL: PASS`, instance VK_SUCCESS, renderer=Mali-G615 MC2, **api=1.3**, vendorID=0x13b5(ARM) — guest→host Vulkan backbone 하드웨어 동작(Vulkan-first 렌더의 토대). 회귀0(GLES3 row-align fix + input routing + GPU self-tests/glmark2 1083/netsurf/foot/sdl2 전부 PASS). **qt6**: machine-id+no-decoration로도 여전히 Qt-init SIGSEGV(게스트 child, 앱 생존) — **EGL hwintegration**(eglGetDisplay, ICD 없음)이 원인 → SHM 강제/EGL 플러그인 비활성 focused fix 필요. evidence: docs/evidence/2026-06-02-round5-vulkan-device-marshal.md. **다음 마일스톤(focused)**: qt6 EGL fix, Vulkan render pipeline(VK-M2 body + guest ICD), **exec-re-entry**(apt/dpkg/GIMP plugin), netsurf 입력/네트워크. (docs/research/loader-feature-gaps.md = 레버리지 SSOT.)
**CP-6 un-defer + M-R2 + drain#15 완료(v137 `1292d3a`)**: 다른 세션이 push한 research/chromium-cp6(ADR-002 raw-svc storm 로드맵 + ADR-003 exec-re-entry + bench/syscall_mix host 프로토타입)를 게이트 머지(pytest 727). 통합 세션이 **ADR-002 M-R2 구현**: supervisor가 기존 2개 trap 사이트에서 syscall-nr 히스토그램(`alr sc trace_hist/emul_hist` top-16) + getrusage(`stime/utime/nonvol_ctxt/traps/emul`) 출력(신규 ptrace op 0). emul backstop 8192→1<<20. **chromium un-defer**(--version). drain#15: **★chromium-headless-shell가 ALR로 in-process 실행**(Chromium 147.0.7727.137, PARSE/MAP/EXEC PASS, exit 0; 186MB+200파일 closure). M-R2 verdict = **mediation-negligible**(traps=0 emul=1 nonvol_ctxt=379 → ALR 중재 라운드트립 0, 917ms는 chromium 자체 init; USER_NOTIF/svc-rewrite는 init 경로엔 ROI 없음). bench classify_storm 퇴화케이스 버그 수정(ratio>4=guest-own, mediation-negligible verdict + 회귀테스트). evidence: docs/evidence/2026-06-02-cp6-mr2-chromium-syscall-mix.md. **다음**: --dump-dom storm verdict는 멀티스레드 ptrace deadlock fix 후(그 뒤 M-R5 svc-rewrite vs M-R1 USER_NOTIF A/B). exec-re-entry(ADR-003)는 apt/dpkg/GIMP-plugin/chromium-multiprocess 공통 잠금.
**round-6 max-parallel fan-out + drain#16 완료(v138 `8b87a95`)**: 서브에이전트 6개(exec-re-entry ADR-003·qt6 EGL-plugin 제외·VK-M2 render body·svc-scanner·compositor popup-grab 스코핑·CP-6 docs) 병렬 게이트 머지 + 통합 세션이 **VK render JNI 배선** + `QT_WAYLAND_DISABLE_HW_INTEGRATION=1`. drain#16: **★Qt6 위젯 앱이 ALR로 렌더** — analogclock `rendered=true`(frames 2215→2216), round-4/5 Qt-init SIGSEGV(wayland **EGL** client-buffer integration → eglGetDisplay ICD 없음)을 **EGL QPA/HwIntegration 플러그인 overlay 제외 + DISABLE_HW_INTEGRATION**으로 해결 → Qt6가 **wl_shm backing store**로 SurfaceView 렌더. 범용 GUI 증명셋 7종 완성(GIMP/gtk3-widget-factory/gtk3-demo/foot/netsurf/SDL2/**Qt6**). **★exec-re-entry(ADR-003 B-1) device-fires**: guest `execve(/bin/sh)`가 EVENT_SECCOMP에서 trap→**x0 path-rewrite**(`alr exec x0=/bin/sh reason=rewrite`, traps=1 rewrites=1 clone_events=7), no-exec guest는 0(회귀0). 단 `apt-install unpacked=false` — dpkg maintainer-script fork+exec 체인은 **B-3 interposer 재주입**(child envp에 LD_PRELOAD/ALR_ROOTFS 전파) 필요(device-only 후속). **VK-M2 render**: guest vkCreateDevice+vkGetDeviceQueue+cmdpool/buf를 실 Mali libvulkan에 마샬→`device created=yes, gfx queue family=0`, but **clear `vkQueueSubmit` fail**(render-pass/image-layout/AHB-import setup 디버그 필요); ENUM은 PASS 유지. 회귀0(GPU LIVE+CUBE PASS, glmark2 1061, foot/gtkdemo/netsurf/sdl2 render, chromium 147 run, no crash). evidence: docs/evidence/2026-06-02-round6-qt6-execreentry-vkrender.md. **다음(focused)**: VK clear-submit fix, exec-re-entry B-3(child 재주입→dpkg/apt+multiprocess), --dump-dom storm deadlock(→M-R5/M-R1 A/B), svc-scan ctor 배선.
**round-7 max-parallel fan-out + drain#17 완료(v139 `0aef81c`+`495e9e8`)**: 서브에이전트 5개(disjoint 소유: alr_gpu VK-render·runtime_report+alr_exec exec-re-entry B-3·alr_interpose svc-scan+child-reentry·alr_wayland input·docs) 병렬 게이트 머지(host gate: pytest 787 + NDK 4-ABI + native-core VK marshal + interpose + shim + wire-check 전부 PASS). drain#17: **★VK-M2 render가 실 Mali에서 PASS** — round-6 `submit=fail`을 R7-A가 해결(마샬 device를 **확장 없이** 생성 → AHB color target import(`VK_ANDROID_external_memory_android_hardware_buffer`)가 unde­fined였음; vkCreateDevice에 AHB device-ext 활성 + tiler readback barrier 추가) → `ALR VK RENDER MARSHAL: PASS`, device created=yes, gfx queue family=0, **clear `vkQueueSubmit`=VK_SUCCESS**. enumerate(round-5)와 합쳐 guest→host Vulkan이 device+queue+cmdbuf+clear-submit까지 실 Mali에서 device-검증. **exec-re-entry B-3**(child envp 재주입): supervisor가 매 execve/execveat trap에서 `decide_exec_envp_injection` 평가 — `/bin/dash`는 `envp_reason=already`(child가 loader-set LD_PRELOAD를 **상속**하므로 주입 불필요, `envp_injected=0`이 정답), B-1 path-rewrite도 계속 fire(`x0=/bin/dash reason=rewrite`). 단 **apt 여전히 unpacked=false** — 차단 원인은 envp가 **아님**: (1)모든 exec에서 `exec_events=0`(rewrite 후 execve가 새 image로 **완료/재진입 안 됨** = exec-completion 벽), (2)apt 자체가 trap 전 `GUEST EXEC FAIL`(apt 최소 staging만). B-3는 정확히 배선·평가 device-확인됐고, apt 결과는 더 깊은 exec-completion 벽 + 불완전 apt staging이 게이트. **svc-scan**(R7-C): ctor 게이트 OFF-by-default 머지+host-build(.so 117KB), guest env에 `ALR_SVCSCAN=1` 배선은 후속(이번 drain 미출력). 회귀0: VK ENUM PASS, GPU LIVE+CUBE PASS, glmark2 1053, foot/gtkdemo/netsurf/**qt6**/sdl2 전부 rendered=true(input focus-follow+axis_source 회귀0), chromium 147 run, no crash. evidence: docs/evidence/2026-06-02-round7-vkrender-pass-drain.md. **다음(focused)**: exec-**completion** 재진입(`exec_events=0` → rewrite된 rootfs 바이너리를 ALR 하에 실제 실행 = 진짜 dpkg/apt 벽), 완전 apt staging, svc-scan env-wire, on-device input 인터랙티비티, --dump-dom storm deadlock.
**round-8 de-risked design fan-out 완료(host-only, no-drain, `72b999e`)**: round-7 가 G1 의 진짜 벽을 **exec-completion 재-맵**(`exec_events=0` = 커널이 glibc PT_INTERP 못 풀어 execve 미완)으로 재구도 → 작동 중인 로더를 **건드리지 않고**(회귀 0) 서브에이전트 3개 병렬: **R8-A** ADR-003-v2 설계(**Option S = static re-entry stub**: 커널이 execve 가능한 정적 ELF → seccomp+SEIZE+envp 상속한 채 glibc 타깃을 in-process 재-맵) + 격리 PoC stub(app/src/main/cpp/alr_reentry/, zig 빌드, **유효 static aarch64 ELF·PT_INTERP 없음** readelf 검증, APK 빌드 불변); **R8-B** 완전 apt/dpkg noble overlay 빌더(tools/build_apt_dpkg_overlay.py — 58-pkg closure 18.7MiB 0-unsat + noble hello.deb 페이로드; **핵심 발견: base 가 이미 dpkg/apt/tar 0755 제공** → `GUEST EXEC FAIL` 은 바이너리 부재가 아니라 R8-A 재-맵 의존 확정); **R8-C** docs G1 재구도 + cp6-status + 11 테스트. host gate: pytest 812 + NDK 4-ABI 불변 + reentry stub static-ELF 검증. **다음(integration round-9, device)**: R8-A 가 준 정확한 배선(runtime_report.cpp execve→stub rewrite ~L2223 + stub 을 rootfs /usr/lib/androlinux/alr-reentry 스테이징)으로 supervisor 에 재-맵 와이어 + **DEVICE-REQ: ALR-R8-reentry** drain(게이트=`exec_events>0` AND EVENT_EXEC 발화 AND stub `ALR-REENTRY: jumping` AND 재진입 게스트가 rootfs 보고 clean exit). R-1 리스크: untrusted_app+stacked-seccomp 에서 static-ELF execve 가 EVENT_EXEC 를 발화하는가 = device-only 미지수(반증 시 Option P fallback).

**exec-re-entry 정복 아크 R9→R11 (v140→v144, integration 단독 + 서브에이전트):** TOP-GOAL 멀티프로세스(apt/dpkg/GIMP-plugin/chromium) 잠금해제의 G1 벽. **R9(v140 `12f21de`)**: Option S(rootfs-경로 정적 stub을 kernel-execve)는 **W^X로 죽음 device-확정** — untrusted_app(targetSdk 35)은 app_data_file을 execve 불가(stub 존재시에도 exec_events=0). **R10 step1(v141 `f7de3fa`)**: ★execve 없는 재진입 **메커니즘 device-증명** — execve 트랩에서 syscall 취소(NT_ARM_SYSTEM_CALL=-1)+PC를 상주 트램폴린(fork-공유 .text)으로 리다이렉트→게스트 안 코드 실행(child exit=123), 커널 execve 0번. **R10 step2(v143 `b7ce619`)**: ★진짜 **map+jump device-증명** — 트램폴린(alr_inproc_reexec.c, alr_reentry.c 재사용)이 타깃 ELF를 in-process mmap(PROT_EXEC, W^X 허용)+점프(`entry=0x400640`), static ELF 경로 추가. **R11(v144 `83baf08`)**: ★재맵 게스트 **SIGILL 해결** — 근인은 map_elf_image가 PT_LOAD마다 MAP_FIXED → 공유 경계 페이지가 두번째 MAP_FIXED에 clobber(첫 세그 code/.rela.plt 소거)→__libc_start_main SIGILL; fix=전체 span 단일 anon 매핑+memcpy+페이지별 union mprotect, 실 HWCAP을 IRELATIVE에 전달. device: 재맵 static /bin/sh가 `signal 4` 0(sp_align=ok, bss_zeroed, hwcap=0x119fff). inproc는 `ALR_REEXEC_INPROC=1` opt-in(기본 OFF) — 전역 ON은 onCreate 프로브 시퀀스 정지(직렬 supervision+/proc/self/exe 엣지); 남은 건 시퀀스 통합(per-exec 스코핑·/proc/self/exe pass-through·apt fakeroot), 개념/매퍼 미지수 아님. 회귀0(GPU LIVE/VK RENDER/SCREEN CUBE PASS, glmark2 1073, GUI 전부 render). **CP-6 chromium-storm(PR #2 머지)**: `--dump-dom` "멀티스레드 데드락"은 **오진** — chromium이 120s 윈도에서 worker clone 도달(clone_events>0); 25s alarm이 무거운 single-init을 죽인 것. 인라인 --dump-dom은 직렬 프로브 경로를 hang → dedicated drain으로 연기. 설계 lineage = ADR-003 v1(상속, R7 반증)→v2 Option S(R9 W^X 죽음)→v3 in-process re-map(R10/R11). evidence: docs/evidence/2026-06-02-round{9,10-step1,10-step2,11}-*.md.
