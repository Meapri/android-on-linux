...
- **WS-2 (GPU, ★유일 크리티컬 패스) — CP-2 software=false + GL context + config(STENCIL=0)✓ + persistent-HostState✓ device-달성(GPU LIVE INTEGRATION/SCREEN CUBE PASS, drain#6):** 다음 = **glmark2 build scene이 Score>0** — 이제 shim이 live GL context까지 도달(GL_RENDERER 뜨고 eglMakeCurrent OK)하지만 **build scene이 vertex shader source 전달 실패로 setup 중단**: glShaderSource 1252B를 셋한 뒤 `glGetShaderiv(GL_SHADER_SOURCE_LENGTH)`=0 → "source length 1252, but got 0" → Score 0. 즉 **shader-source 마샬링**(glShaderSource payload를 ring으로 host real GL에 복사 + GL_SHADER_SOURCE_LENGTH 응답)이 다음 관문. (evidence: docs/evidence/2026-06-02-cp2-glmark2-persist-hoststate-shader-source-gap.md.) = **CP-2 최종 = GPU 풀가속 마지막 관문**.
...
- **WS-5 (verify/bench) — CP-1✓ CP-3✓(microbench A/B) CP-4✓ + gtk3-GUI evidence:** 다음 = (a) **CP-2 glmark2 Score 파싱**(WS-2 shader-source fix 후 — 현재 Score 0, setup-fail이라 파싱 보류), (b) compat-matrix(gtk3demo 2213f/foot/gtk3-widget-factory RAN/gimp 3.0.2 device-render 확인됨), (c) CP-3 ratio 문서화.
