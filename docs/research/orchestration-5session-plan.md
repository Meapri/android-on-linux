...
- **WS-4 (rootfs) — .so-x-bit✓ C.UTF-8(noble)✓ SVG-loader✓ toolkit-noble(qt6/netsurf)✓ overlay-stage device✓(drain#8: dpkg-db 195/x11 131/apt-config 2 extract, guard 0 skip):** 다음 = (a) **passwd/nss(`getpwuid` uid 10326) — drain#8서 여전히 warn**(gtk3-widget-factory 비치명적), (b) apt/X11 **기능** 검증(현재 overlay stage만 증명; 실제 apt/dpkg query·X11 client 실행은 실행요약 TextView로만 — logcat 프로브 필요), (c) babl/gegl `.so`, qt6/netsurf device 검증, xwayland auto-stage 목록 추가(§line163).
- **WS-5 (verify/bench) — CP-1✓ CP-3✓(microbench A/B) CP-4✓ CP-2✓(glmark2 Score 1074 파싱) + gtk3-GUI evidence:** 다음 = (a) CP-2 ratio(ALR glmark2 vs Mali 직접) + 전체 scene 매트릭스(8MiB ring APK 후), (b) compat-matrix(gtk3demo 2213f/foot/gtk3-widget-factory RAN/gimp 3.0.2 device-render), (c) CP-3 ratio 문서화.
...
**CP-5 batch drain#8 완료**: ws-2 147e163(중복)은 `-s ours`로 supersede(검증본 997a1c0 유지) → main `2144c6f`. 그 뒤 stamp v129(`5fa5549`)로 APK 재빌드(8MiB ring + ws-3 present-fallback + ws-4 overlays 전부 device 반영) + glmark2 build+texture. drain#8: texture 1123 FPS/Score 1163, ws-4 overlays(dpkg/x11/apt-config) stage clean, GPU self-tests PASS, 회귀0. **남은 device 잔여**: ws-4 apt/X11 **기능** 운용(현 overlay stage만), getpwuid uid 10326, 전체 glmark2 scene 매트릭스, toolkit(sdl2/netsurf/qt6) launch.
