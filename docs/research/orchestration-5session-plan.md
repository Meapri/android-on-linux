...
**CP-2 달성 → 5-WS 크리티컬 패스 전부 CLOSED.** glmark2가 ALR shim→ring→host Mali executor로 1075 FPS 렌더(Score 1074, software=false, drain#7). CPU(CP-3 0%)·display(90Hz)·present(CP-4)·GUI(gtk3/foot/GIMP)·GPU(glmark2 Mali) 전 축 device-증명 완료. 남은 건 전부 폭(범용성) 확장 — texture-scene 8MiB ring APK, ws-4 apt/Xwayland/dpkg device 운용, GLES3+/Vulkan, 입력/멀티윈도우 마감.
...
**CP-2 drain#7 완료**: shim shader-source fix(`997a1c0`)를 rebuilt gpushim-stage.tar(268800)로 re-stage(APK 미재빌드 — shim은 APK에 없음, stage-tar로 배포). glmark2 Score 1074, GPU self-tests PASS. **device 배치 잔여**: ws-2 8MiB ring·ws-3 present-fallback·ws-4 overlays는 현 device APK(pre-merge v128)에 미반영 → texture-scene/apt/X11 운용 검증 시 APK 재빌드(`e6d513f+`, stamp bump) 후 1회 batch drain. build-scene CP-2는 현 APK로 충족.
