...
class MainActivity : Activity() {
    ...
    /**
     * A [dev.chanwoo.androlinux.runtime.AptInstaller.Host] backed by THIS Activity's JNI
     * exports + Context. The marker probe and (separately) NativeAlrRuntime both drive the
     * SAME shared pipeline through their own Host — identical native behavior, different
     * entry symbol (MainActivity-mangled here, AlrNative-mangled in the runtime path).
     */
    private fun aptInstallerHost() = object : dev.chanwoo.androlinux.runtime.AptInstaller.Host {
        override val packageName: String get() = this@MainActivity.packageName
        override val nativeLibraryDir: String get() = applicationInfo.nativeLibraryDir
        override val filesDir: String get() = this@MainActivity.filesDir.absolutePath
        override val cacheDir: String get() = this@MainActivity.cacheDir.absolutePath
    ...
    private fun runAptInstallProbe(rootfsDir: File, rootfsName: String, pkg: String) {
        // The marker keystone now delegates to the SHARED AptInstaller (the extraction of
        // this very probe), so MainActivity's `echo install:<pkg>` proof and the in-app
        // launcher install() run byte-identical native steps. The proof logcat lines
        // ("aptinstall: …", "aptinstall-fetch: Get: …", "Setting up <pkg>", FINAL …) are
        // emitted by AptInstaller itself — same tag, same text — so existing log greps hold.
        Thread {
            try {
                dev.chanwoo.androlinux.runtime.AptInstaller.install(
                    host = aptInstallerHost(),
                    rootfsDir = rootfsDir,
