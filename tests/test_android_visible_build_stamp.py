...
def test_debug_apk_version_code_is_bumped_for_device_update():
    ...
    assert "versionCode = 132" in text
    assert 'versionName = "0.4.132-breadth-fanout-v132"' in text


def test_main_activity_starts_with_visible_build_stamp_before_summary():
    text = MAIN.read_text()
    assert "build: 0.4.132-breadth-fanout-v132" in text
    assert text.index("build: 0.4.132-breadth-fanout-v132") < text.index("execution summary")
